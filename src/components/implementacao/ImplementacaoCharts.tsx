...
interface ImplementacaoChartsProps {
  despesas: any[];
  receitas: any[];
}

const ImplementacaoCharts: React.FC<ImplementacaoChartsProps> = ({ despesas, receitas }) => {
  ...
  // Filtrar dados apenas para Implementação para evolução mensal
  const despesasImplementacao = React.useMemo(() => {
  ...
  const evolucaoMensal = React.useMemo(() => {
    ...
    return months.map((month, index) => {
      const monthDespesas = despesasImplementacao.filter(d => {
      ...
      const monthReceitas = receitasImplementacao.filter(r => {
...
export default ImplementacaoCharts;
